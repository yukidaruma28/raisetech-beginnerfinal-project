...
| 本番 | `http://54.64.68.36:8080/api` （nginx がポート 8080 で受けて Rails コンテナへ転送）|
...
| `PATCH /api/statuses/:id` | ⏳ 未実装（UC-07 で実装予定）|
...
| PATCH | /statuses/:id | ステータス更新（名前・色）❌ **MVP スコープ外・未実装**。シングルユーザー運用のため UI からの編集機能は提供しない。 | `{ name?, color? }` | `Status` |
